...
Якщо число Фібоначчі зберігається у словнику cache, то функція fibonacci повертає число з кеша. Якщо його немає у кеші, 
то ми обчислюємо число і поміщаємо його в кеш, і повертаємо з функції fibonacci.

"""
 
def caching_fibonacci():
    cache = {}  # Створимо кеш для збереження обчислених значень

    def fibonacci(n):
        if n in cache:  # Перевіряємо, чи є число Фібоначчі у кеші
            return cache[n]
        ...
        if n <= 0:
            ...
            # Обчислюємо число Фібоначчі рекурсивно і зберігаємо його у кеші
...
print(fib(5))  # Виведе 5, тому що п'ятий член ряду Фібоначчі - 5
print(fib(10)) # Виведе 55, тому що десятий член ряду Фібоначчі - 55